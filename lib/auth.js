...
function sanitizeUser(user, { brief = false } = {}) {
  const base = {
    id: user.id,
    name: user.name,
    email: user.email,
  ...
  base.avatar = user.avatar && user.avatar.length <= 120000 ? user.avatar : null;
  base.cover = user.cover && user.cover.length <= 120000 ? user.cover : null;
  return base;
}
...
async function updateProfile(userId, { name, avatar, cover }) {
  const data = loadUsers();
  const idx = data.users.findIndex((u) => u.id === userId);
...
async function mergeUsersImport(incoming) {
  ...
  for (const raw of incoming.users) {
    ...
    if (existing) {
      ...
      if (raw.cover && raw.cover.length <= 900000) existing.cover = raw.cover;
      ...
      if (raw.cover && raw.cover.length <= 900000) user.cover = raw.cover;
      data.users.push(user);
