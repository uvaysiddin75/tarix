...
function normalizeUsersData(data) {
  ...
  for (const user of data.users) {
    if (!user.passwordHash && user.passwordPlain) {
      user.passwordHash = bcrypt.hashSync(user.passwordPlain, SALT_ROUNDS);
    }
...
async function init() {
  ...
  memory = loadFromDisk();
  memory = normalizeUsersData(memory);

  if (!memory.users?.length) {
    try {
      const raw = await loadFromGitHubRaw();
      memory = normalizeUsersData(raw);
      if (memory.users?.length) {
    ...
    // Diskda kam bo'lsa, GitHub dagi to'liqroq bazani olish
    try {
      const raw = normalizeUsersData(await loadFromGitHubRaw());
      if (raw.users.length > memory.users.length) {
        memory = raw;
        fs.writeFileSync(USERS_PATH, JSON.stringify(memory, null, 2), "utf8");
        console.log(`GitHub repodan yangilandi (${memory.users.length} ta)`);
      }
    } catch {
      /* ignore */
    }
